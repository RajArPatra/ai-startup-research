...
// ── Init ──
async function init() {
    const manifest = await fetchJSON('research-data/index.json');
    if (!manifest || !manifest.weeks.length) {
        document.getElementById('report-content').innerHTML =
            '<div class="loading">No reports yet. Run the research scraper first.</div>';
        return;
    }
...
async function loadWeek(date, topicId) {
    ...
    const data = await fetchJSON(`research-data/${date}.json`);
    if (!data) {
